...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x,lora_q=None,lora_k=None,lora_v=None,lora_o=None):
		...
		q_pre=self.c_q(x);k_pre=self.c_k(x);v_pre=self.c_v(x)
		if lora_q is not None:q_pre=q_pre+apply_lora_delta(x,lora_q[0].to(dtype=x.dtype),lora_q[1].to(dtype=x.dtype))
		if lora_k is not None:k_pre=k_pre+apply_lora_delta(x,lora_k[0].to(dtype=x.dtype),lora_k[1].to(dtype=x.dtype))
		if lora_v is not None:v_pre=v_pre+apply_lora_delta(x,lora_v[0].to(dtype=x.dtype),lora_v[1].to(dtype=x.dtype))
		q=q_pre.reshape(bsz,seqlen,self.num_heads,self.head_dim);k=k_pre.reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);v=v_pre.reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);q=F.rms_norm(q,(q.size(-1),));k=F.rms_norm(k,(k.size(-1),));cos,sin=self.rotary(seqlen,x.device,q.dtype);q=apply_rotary_emb(q,cos,sin,self.rope_dims);k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,None,:,None];y=flash_attn_3_func(q,k,v,causal=True)
		if self.use_xsa:y=self._xsa_efficient(y,v)
		y=y.reshape(bsz,seqlen,dim);y_out=self.proj(y)
		if lora_o is not None:y_out=y_out+apply_lora_delta(y,lora_o[0].to(dtype=y.dtype),lora_o[1].to(dtype=y.dtype))
		return y_out
class MLP(nn.Module):
...
class SlotLoRABank(nn.Module):
	"""Per-doc, per-slot LoRA banks for batched TTT on a recurrent stack.
	Banks are indexed by (doc_in_batch, slot_in_unrolled_schedule) so that
	reused blocks get distinct adapters at distinct call sites. Stays None
	for slots not in active_slots (set during attach)."""
	def __init__(self,max_bsz,num_slots,dim,kv_dim,rank,proj_set,init_scale,active_slots,dtype=torch.float32):
		super().__init__();self.max_bsz=max_bsz;self.num_slots=num_slots;self.dim=dim;self.kv_dim=kv_dim;self.rank=rank;self.proj_set=tuple(proj_set);self.active_slots=set(int(s)for s in active_slots);self.init_scale=init_scale
		self.A_q=nn.Parameter(torch.zeros(max_bsz,num_slots,rank,dim,dtype=dtype))if'q'in self.proj_set else None
		self.B_q=nn.Parameter(torch.zeros(max_bsz,num_slots,dim,rank,dtype=dtype))if'q'in self.proj_set else None
		self.A_k=nn.Parameter(torch.zeros(max_bsz,num_slots,rank,dim,dtype=dtype))if'k'in self.proj_set else None
		self.B_k=nn.Parameter(torch.zeros(max_bsz,num_slots,kv_dim,rank,dtype=dtype))if'k'in self.proj_set else None
		self.A_v=nn.Parameter(torch.zeros(max_bsz,num_slots,rank,dim,dtype=dtype))if'v'in self.proj_set else None
		self.B_v=nn.Parameter(torch.zeros(max_bsz,num_slots,kv_dim,rank,dtype=dtype))if'v'in self.proj_set else None
		self.A_o=nn.Parameter(torch.zeros(max_bsz,num_slots,rank,dim,dtype=dtype))if'o'in self.proj_set else None
		self.B_o=nn.Parameter(torch.zeros(max_bsz,num_slots,dim,rank,dtype=dtype))if'o'in self.proj_set else None
		self.reset_state()
	@torch.no_grad()
	def reset_state(self):
		"""LoRA standard init: A=small Kaiming, B=0 so initial delta is 0."""
		for A in(self.A_q,self.A_k,self.A_v,self.A_o):
			if A is not None:nn.init.kaiming_uniform_(A.view(-1,A.size(-1)),a=math.sqrt(5));A.mul_(self.init_scale)
		for B in(self.B_q,self.B_k,self.B_v,self.B_o):
			if B is not None:B.zero_()
	def slot_active(self,slot_id):return int(slot_id)in self.active_slots
	def get(self,proj,slot_id,bsz):
		"""Return (A_slot, B_slot) sliced to current batch size, or None if proj not present."""
		A=getattr(self,f'A_{proj}',None);B=getattr(self,f'B_{proj}',None)
		if A is None or B is None:return None
		if not self.slot_active(slot_id):return None
		return(A[:bsz,slot_id],B[:bsz,slot_id])
def apply_lora_delta(x,A,B):
	"""x: [B,T,d_in]. A: [B,r,d_in]. B: [B,d_out,r]. Returns [B,T,d_out]."""
	xA=torch.einsum('btd,brd->btr',x,A);return torch.einsum('btr,bdr->btd',xA,B)
class Block(nn.Module):
	...
	def forward(self,x,x0,lora_q=None,lora_k=None,lora_v=None,lora_o=None):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=self.attn(self.attn_norm(x_in)*self.ln_scale_factor,lora_q=lora_q,lora_k=lora_k,lora_v=lora_v,lora_o=lora_o)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self.ttt_lora_bank=None;self.ttt_lora_bsz=0;self._init_weights()
	...
	def _slot_lora(self,slot_id):
		"""Return per-projection LoRA tuples for a slot, or all-None if no bank or slot inactive.
		bsz is read from self.ttt_lora_bsz which the caller must set before forward."""
		bank=self.ttt_lora_bank
		if bank is None or self.ttt_lora_bsz<=0:return(None,None,None,None)
		bsz=self.ttt_lora_bsz
		return(bank.get('q',slot_id,bsz),bank.get('k',slot_id,bsz),bank.get('v',slot_id,bsz),bank.get('o',slot_id,bsz))
	def forward_logits(self,input_ids):
		...
		enc_list=list(enc_iter);dec_list=list(dec_iter);num_enc_slots=len(enc_list)
		for(slot_id,i)in enumerate(enc_list):
			lq,lk,lv,lo=self._slot_lora(slot_id);x=self.blocks[i](x,x0,lora_q=lq,lora_k=lk,lora_v=lv,lora_o=lo);skips.append(x)
		for(skip_idx,i)in enumerate(dec_list):
			...
			slot_id=num_enc_slots+skip_idx;lq,lk,lv,lo=self._slot_lora(slot_id);x=self.blocks[i](x,x0,lora_q=lq,lora_k=lk,lora_v=lv,lora_o=lo)
		x=self.final_norm(x)
	...
	def attach_ttt_lora_bank(self,h,device,active_slot_ids,proj_set,rank=None,max_bsz=None,init_scale=None):
		"""Build and attach a SlotLoRABank sized to the current schedule. Idempotent: replaces any prior bank.
		active_slot_ids: iterable of slot positions in the unrolled schedule (0..num_slots-1) to allocate.
		proj_set: subset of {'q','k','v','o'}.
		Returns the bank for caller to register with optimizer."""
		num_slots=len(self.encoder_indices)+len(self.decoder_indices);dim=h.model_dim;kv_dim=h.num_kv_heads*(h.model_dim//h.num_heads)
		r=h.ttt_lora_rank if rank is None else rank;mb=h.ttt_lora_max_bsz if max_bsz is None else max_bsz;sc=h.ttt_lora_init_scale if init_scale is None else init_scale
		bank=SlotLoRABank(mb,num_slots,dim,kv_dim,r,proj_set,sc,active_slot_ids).to(device)
		self.ttt_lora_bank=bank;self.ttt_lora_bsz=0;return bank
	def detach_ttt_lora_bank(self):self.ttt_lora_bank=None;self.ttt_lora_bsz=0
...
def _select_ttt_params(base_model,h,device):
	"""Select which parameters get gradient during TTT, based on env config.
	Returns (ttt_params, layer_ids, mode_str, lora_bank_or_None, lora_slot_ids)."""
	layer_ids=[int(x)for x in h.ttt_layer_ids.split(',')if x.strip()]if h.ttt_layer_ids else None
	if layer_ids is None:
		if h.ttt_selective_layers>0:layer_ids=list(range(max(0,h.num_layers-h.ttt_selective_layers),h.num_layers))
		else:layer_ids=list(range(h.num_layers))
	for p in base_model.parameters():p.requires_grad_(False)
	ttt_params=[];mode=h.ttt_mode
	for i in layer_ids:
		b=base_model.blocks[i]
		if mode=='control':
			for(name,p)in b.named_parameters():
				if any(pat in name for pat in CONTROL_TENSOR_NAME_PATTERNS):ttt_params.append(p)
		elif mode=='qv':ttt_params.extend([b.attn.c_q.weight,b.attn.c_v.weight])
		elif mode=='qkv':ttt_params.extend([b.attn.c_q.weight,b.attn.c_k.weight,b.attn.c_v.weight])
		elif mode=='full':ttt_params.extend(list(b.parameters()))
		elif mode=='none':pass
		else:raise ValueError(f"unknown TTT_MODE: {mode}")
	if h.ttt_include_global:
		if base_model.skip_weights.numel()>0:ttt_params.append(base_model.skip_weights)
		if base_model.skip_gates is not None and base_model.skip_gates.numel()>0:ttt_params.append(base_model.skip_gates)
	bank=None;lora_slot_ids=[]
	if h.ttt_lora_enabled:
		if h.ttt_lora_slot_ids:lora_slot_ids=[int(x)for x in h.ttt_lora_slot_ids.split(',')if x.strip()]
		else:
			schedule=list(base_model.encoder_indices)+list(base_model.decoder_indices);target=set(layer_ids);lora_slot_ids=[s for(s,bid)in enumerate(schedule)if bid in target]
		proj_set=tuple(p.strip()for p in h.ttt_lora_proj.split(',')if p.strip())
		bank=base_model.attach_ttt_lora_bank(h,device,lora_slot_ids,proj_set);ttt_params.extend(list(bank.parameters()))
	for p in ttt_params:p.requires_grad_(True)
	return ttt_params,layer_ids,mode,bank,lora_slot_ids
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	ttt_params,layer_ids,mode,bank,lora_slot_ids=_select_ttt_params(base_model,h,device)
	use_compile=bank is None
	if use_compile:score_fwd=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True)
	else:score_fwd=base_model.forward_logits
	ttt_fwd=base_model
	...
	for ci in range(num_chunks):
		...
		chunk_start=ci*ttt_chunk;chunk_end=min((ci+1)*ttt_chunk,total_tokens);my_s=len(windows)*rank//world_size;my_e=len(windows)*(rank+1)//world_size;my_windows=windows[my_s:my_e];base_model.train(False)
		with torch.no_grad():
			for bi in range(0,len(my_windows),batch_seqs):
				batch_ws=my_windows[bi:bi+batch_seqs];bsz=len(batch_ws);x_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);y_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);wlens=[]
				...
				if bank is not None:base_model.ttt_lora_bsz=bsz
				if phase_log:s_evt=_evt()
				with torch.autocast(device_type='cuda',dtype=torch.bfloat16):logits=score_fwd(x_batch)
				if phase_log:score_evs.append((s_evt,_evt()))
				nll=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),y_batch.reshape(-1),reduction='none').reshape(bsz,seq_len)
				for(i,ws)in enumerate(batch_ws):wlen=wlens[i];s=0 if ws==0 else context_size;scored_nll=nll[i,s:wlen].to(torch.float64);loss_sum+=scored_nll.sum();token_count+=float(wlen-s);tgt=y_batch[i,s:wlen];prev=x_batch[i,s:wlen];tb=val_data.base_bytes_lut[tgt].to(torch.float64);tb+=(val_data.has_leading_space_lut[tgt]&~val_data.is_boundary_token_lut[prev]).to(torch.float64);byte_count+=tb.sum()
		is_last_chunk=ci==num_chunks-1
		if not is_last_chunk and h.ttt_epochs>0 and optimizer is not None:
			...
			if chunk_seqs>0:
				...
				for _ep in range(h.ttt_epochs):
					for bs in range(0,my_chunk_seqs,batch_seqs):
						...
						if bank is not None:base_model.ttt_lora_bsz=x.size(0)
						if phase_log:f_evt=_evt()
						with torch.autocast(device_type='cuda',dtype=torch.bfloat16):loss=ttt_fwd(x,y)
						if phase_log:fwd_evs.append((f_evt,_evt()));b_evt=_evt()
						loss.backward()
						if phase_log:bwd_evs.append((b_evt,_evt()))
						if world_size>1:
							if phase_log:r_evt=_evt()
							for p in ttt_params:
								if p.grad is not None:dist.all_reduce(p.grad,op=dist.ReduceOp.AVG)
							if phase_log:red_evs.append((r_evt,_evt()))
						if phase_log:o_evt=_evt()
						torch.nn.utils.clip_grad_norm_(ttt_params,1.);optimizer.step()
						if phase_log:opt_evs.append((o_evt,_evt()))
	if dist.is_available()and dist.is_initialized():dist.all_reduce(loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(token_count,op=dist.ReduceOp.SUM);dist.all_reduce(byte_count,op=dist.ReduceOp.SUM)
	if phase_log:
		torch.cuda.synchronize()
		def _ms(evs):return sum(s.elapsed_time(e)for(s,e)in evs)
		log(f"ttt:phase_ms score={_ms(score_evs):.0f} ttt_fwd={_ms(fwd_evs):.0f} backward={_ms(bwd_evs):.0f} all_reduce={_ms(red_evs):.0f} optimizer={_ms(opt_evs):.0f} score_n={len(score_evs)} ttt_n={len(fwd_evs)}")
	for p in base_model.parameters():p.requires_grad_(True)
	if bank is not None:base_model.detach_ttt_lora_bank()
	base_model.train(False);return _loss_bpb(loss_sum,token_count,byte_count)
...
def train_model(h,device,val_data):
	...
	def step_fn(step,lr_scale):
		...
		if h.grad_clip_norm>0:
			muon_ids={id(p) for grp in optimizers.optimizer_muon.param_groups for p in grp['params']}
			non_muon=[p for p in base_model.parameters() if id(p) not in muon_ids and p.grad is not None]
			if non_muon:torch.nn.utils.clip_grad_norm_(non_muon,h.grad_clip_norm)
