...
class MLP(nn.Module):
 ...
 def forward(self, x: Tensor) -> Tensor:
  h = F.leaky_relu(self.fc1(x), negative_slope=0.5)
  x = x + self.proj1(h.square())
  h = F.leaky_relu(self.fc2(x), negative_slope=0.5)
...
class Block(nn.Module):
 def __init__(
  ...
  self.ddl_w = nn.Parameter(torch.randn(dim, dtype=torch.float32) * 0.01)
  self.ddl_v = nn.Parameter(torch.randn(dim, dtype=torch.float32) * 0.01)
 ...
 def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, v_first: Tensor | None = None) -> tuple[Tensor, Tensor]:
  ...
  beta = torch.sigmoid(self.ddl_beta.to(dtype=x_in.dtype))
  erase = beta * (x_in * self.ddl_w.to(dtype=x_in.dtype)[None, None, :]).sum(dim=-1, keepdim=True) * self.ddl_v.to(dtype=x_in.dtype)[None, None, :]
  x_in = x_in - erase
 ...
 with torch.inference_mode():
  for bi in range(0, len(my_windows), batch_seqs):
   ...
   if cache_counts.sum() > 0:
    cache_probs = cache_counts / cache_counts.sum()
    model_probs = torch.softmax(logits_f, dim=-1)
    mixed_probs = (1 - args.cache_lambda) * model_probs + args.cache_lambda * cache_probs[None, None, :]
    nll = -torch.log(mixed_probs.gather(-1, y_batch.unsqueeze(-1)).squeeze(-1).clamp(min=1e-10))
   else:
   ...
   for i, ws in enumerate(batch_ws):
    ...
    for t in range(s, wlen):
     ...
     pred_probs = torch.softmax(logits_f[i, t], dim=-1)
     ogd_grad = pred_probs.clone()
     ogd_grad[tid] -= 1.0
     ogd_bias = ogd_bias - args.ogd_lr * ogd_grad
...
def main() -> None:
 ...
 if args.ogd_enabled:
  ...
  ogd_val_loss, ogd_val_bpb = eval_val_ogd(
   args, eval_model, rank, world_size, device,
   val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
  ...
  torch.cuda.synchronize()
  log0(
   f"final_ogd val_loss:{ogd_val_loss:.4f} val_bpb:{ogd_val_bpb:.4f} "
   f"ogd_lr:{args.ogd_lr} cache_lambda:{args.cache_lambda} eval_time:{1000.0 * (time.perf_counter() - t_ogd):.0f}ms"
  )
  log0(f"final_ogd_exact val_loss:{ogd_val_loss:.8f} val_bpb:{ogd_val_bpb:.8f}")
 if distributed:
