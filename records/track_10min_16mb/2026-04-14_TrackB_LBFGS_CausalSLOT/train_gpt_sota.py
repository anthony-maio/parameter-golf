...
def eval_val_lbfgs_causal_slot(h,device,val_data,base_model,batch_seqs=32):
	base_model.train(False);logits_fn=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);seq_len=h.eval_seq_len;context_size=seq_len-h.eval_stride;total_tokens=val_data.val_tokens.numel()-1;window_starts=[ws for ws in range(0,total_tokens,h.eval_stride)if ws+context_size<total_tokens];total_windows=len(window_starts);my_s=total_windows*h.rank//h.world_size;my_e=total_windows*(h.rank+1)//h.world_size;my_windows=window_starts[my_s:my_e];loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);vocab=h.vocab_size;dim=h.model_dim
	log(f"causal_slot:start lbfgs_steps={h.causal_slot_lbfgs_steps} max_iter={h.causal_slot_lbfgs_max_iter} lr={h.causal_slot_lbfgs_lr} use_delta={h.causal_slot_use_delta} use_bias={h.causal_slot_use_logit_bias} batch_seqs={batch_seqs}")
	for bi in range(0,len(my_windows),batch_seqs):
		batch_ws=my_windows[bi:bi+batch_seqs];bsz=len(batch_ws);x_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);y_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);wlens=[]
		for(i,ws)in enumerate(batch_ws):we=min(ws+seq_len,total_tokens);wlen=we-ws;wlens.append(wlen);chunk=val_data.val_tokens[ws:we+1].to(dtype=torch.int64,device=device);x_batch[i,:wlen]=chunk[:-1];y_batch[i,:wlen]=chunk[1:]
		with torch.inference_mode():
			with torch.autocast(device_type='cuda',dtype=torch.bfloat16):base_logits_bf16=logits_fn(x_batch)
		base_logits=base_logits_bf16.detach().clone().float()
		params_to_opt=[]
		logit_bias=None;delta=None
		if h.causal_slot_use_logit_bias:logit_bias=torch.zeros(bsz,1,vocab,device=device,dtype=torch.float32,requires_grad=True);params_to_opt.append(logit_bias)
		if h.causal_slot_use_delta:delta=torch.zeros(bsz,1,dim,device=device,dtype=torch.float32,requires_grad=True);params_to_opt.append(delta)
		context_mask=torch.zeros(bsz,seq_len,device=device,dtype=torch.float32)
		any_context=False
		for(i,ws)in enumerate(batch_ws):
			if ws==0:continue
			context_mask[i,0:context_size]=1.0;any_context=True
		if any_context and params_to_opt:
			optimizer=torch.optim.LBFGS(params_to_opt,lr=h.causal_slot_lbfgs_lr,max_iter=h.causal_slot_lbfgs_max_iter,tolerance_grad=1e-7,tolerance_change=1e-9,history_size=10,line_search_fn='strong_wolfe')
			def closure():
				optimizer.zero_grad()
				biased=base_logits
				if logit_bias is not None:biased=biased+logit_bias
				if delta is not None:
					tok_emb_w=base_model.tok_emb.weight.to(torch.float32);delta_logits=F.linear(delta,tok_emb_w) if base_model.tie_embeddings else F.linear(delta,base_model.lm_head.weight.to(torch.float32));biased=biased+delta_logits
				nll=F.cross_entropy(biased.reshape(-1,vocab),y_batch.reshape(-1),reduction='none').reshape(bsz,seq_len)
				ctx_count=context_mask.sum().clamp_min(1.0)
				loss=(nll*context_mask).sum()/ctx_count
				loss.backward()
				return loss
			for _ in range(h.causal_slot_lbfgs_steps):optimizer.step(closure)
		with torch.no_grad():
			biased=base_logits
			if logit_bias is not None:biased=biased+logit_bias
			if delta is not None:
...
def train_and_eval(h,device):
	...
	if h.causal_slot_enabled and h.sliding_window_enabled:timed_eval('quantized_causal_slot_lbfgs',eval_val_lbfgs_causal_slot,h,device,val_data,eval_model)
