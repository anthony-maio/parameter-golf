...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.bigram_buckets=h.bigram_buckets if h.bigram_enabled and h.bigram_buckets>0 else 0;self.bigram_hash_seed=h.bigram_hash_seed
		if self.bigram_buckets>0:
			self.bigram_emb=nn.Embedding(h.bigram_buckets,h.bigram_dim)
			self.bigram_proj=CastedLinear(h.bigram_dim,h.embedding_dim,bias=False) if h.bigram_dim!=h.embedding_dim else None
		else:self.bigram_emb=None;self.bigram_proj=None
		self._init_weights()
	...
	def forward_logits(self,input_ids):
		...
		if self.bigram_emb is not None:
			prev=F.pad(input_ids[:,:-1],(1,0),value=0);bigram_ids=(prev*self.bigram_hash_seed+input_ids)%self.bigram_buckets
			b=self.bigram_emb(bigram_ids)
			if self.bigram_proj is not None:b=self.bigram_proj(b)
			x=x+b
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		if not t.is_floating_point()or t.numel()<=65536 or'bigram_emb'in name or'bigram_proj'in name:result[name]=t.to(torch.float16)if t.is_floating_point()else t;meta[name]='passthrough (float16)';continue
		if'tok_emb'in name:cs=h.embed_clip_sigmas;bits=h.embed_bits
		elif'.mlp.'in name:cs=h.matrix_clip_sigmas;bits=h.mlp_bits
