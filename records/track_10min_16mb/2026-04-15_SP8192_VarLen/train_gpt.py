...
class Hyperparameters:
    ...
    use_varlen = bool(int(os.environ.get('USE_VARLEN', '1')))
    use_fused_mlp = bool(int(os.environ.get('USE_FUSED_MLP', '1')))
    cu_bucket_size = int(os.environ.get('CU_BUCKET_SIZE', 64))
    max_doc_len = int(os.environ.get('MAX_DOC_LEN', 2048))
...
def _build_cu_seqlens(bos_pos, total_len, device, max_doc_len=0, bucket_size=64):
    ...
    seg_starts = []
    starts_with_end = list(bos_pos) + [total_len]
...
class DocumentPackingLoader:
    ...
    def __init__(self, h, device, cu_bucket_size=64):
        ...
        self.max_doc_len = h.max_doc_len
    ...
    def _prepare_batch(self, num_tokens_local, max_seq_len):
        ...
        cu_seqlens, max_seqlen = _build_cu_seqlens(starts, inputs.numel(), inputs.device, self.max_doc_len, self.cu_bucket_size)
    ...
    def next_batch(self, global_tokens, grad_accum_steps):
        ...
        if self._next_batch is not None:
            ...
            inputs, targets, cu_seqlens, max_seqlen = self._prepare_batch(num_tokens_local, self.max_seq_len)
        self._next_batch = self._batch_pool.submit(self._prepare_batch, num_tokens_local, self.max_seq_len)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, cu_seqlens=None, max_seqlen=0):
        ...
        if cu_seqlens is not None:
            y = flash_attn_varlen_func(q[0], k[0], v[0],
                                       cu_seqlens_q=cu_seqlens, cu_seqlens_k=cu_seqlens,
                                       max_seqlen_q=max_seqlen, max_seqlen_k=max_seqlen,
                                       causal=True, window_size=(-1, -1))[None]
        else:
...
@triton.jit
def linear_leaky_relu_square_kernel(a_desc, b_desc, c_desc, aux_desc, M, N, K,
...
def linear_leaky_relu_square(a, b, aux=None):
    ...
    linear_leaky_relu_square_kernel[grid](a_desc, b_desc, c_desc, aux_desc, M, N, K,
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        if self.training and self.use_fused:
            return FusedLeakyReLUSquareMLP(x, self.fc.weight.to(x.dtype), self.proj.weight.to(x.dtype))
...
class Block(nn.Module):
    ...
    def forward(self, x, x0, cu_seqlens=None, max_seqlen=0):
        ...
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
        if self.parallel:
...
def train_model(h, device, val_data):
    ...
    train_loader = DocumentPackingLoader(h, device, cu_bucket_size=h.cu_bucket_size) if h.use_varlen else ShuffledSequenceLoader(h, device)
