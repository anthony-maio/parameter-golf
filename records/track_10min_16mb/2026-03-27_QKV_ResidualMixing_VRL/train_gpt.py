...
class Hyperparameters:
 ...
 qkrl_enabled = bool(int(os.environ.get("QKRL_ENABLED", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
 ...
 for pattern in os.environ.get(
  ...
  "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,qrl_gate,krl_gate",
 ).split(",")
 if pattern
)
...
class CausalSelfAttention(nn.Module):
 def __init__(
  ...
  self.qrl_gate = None  # Q residual learning gate
  self.krl_gate = None  # K residual learning gate
 ...
 def forward(self, x: Tensor, v_embed: Tensor | None = None, v_first: Tensor | None = None,
       q_first: Tensor | None = None, k_first: Tensor | None = None) -> tuple[Tensor, Tensor, Tensor, Tensor]:
  ...
  if q_first is not None and self.qrl_gate is not None:
   qg = torch.sigmoid(self.qrl_gate.to(dtype=q.dtype))
   q = (1 - qg) * q + qg * F.rms_norm(q_first, (q_first.size(-1),))
  if k_first is not None and self.krl_gate is not None:
   kg = torch.sigmoid(self.krl_gate.to(dtype=k.dtype))
   k = (1 - kg) * k + kg * F.rms_norm(k_first, (k_first.size(-1),))
  ...
  return self.proj(y), v_raw, q_raw, k_raw
...
class Block(nn.Module):
 ...
 def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None,
       v_first: Tensor | None = None, q_first: Tensor | None = None, k_first: Tensor | None = None) -> tuple[Tensor, Tensor, Tensor, Tensor]:
  mix = self.resid_mix.to(dtype=x.dtype)
  x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
  attn_out, v_raw, q_raw, k_raw = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, v_embed=v_embed, v_first=v_first, q_first=q_first, k_first=k_first)
  x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
  x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor)
  if self.dtg_gate is not None:
   gate = torch.sigmoid(self.dtg_gate(x_in.detach()))
   x_out = x_in + gate * (x_out - x_in)
  return x_out, v_raw, q_raw, k_raw
class GPT(nn.Module):
 def __init__(
  self,
  vocab_size: int,
  ...
  self.qkrl_enabled = qkrl_enabled
  if vrl_enabled:
   for i in range(1, num_layers):
    self.blocks[i].attn.vrl_gate = nn.Parameter(torch.tensor(-1.5, dtype=torch.float32))
  if qkrl_enabled:
   for i in range(1, num_layers):
    self.blocks[i].attn.qrl_gate = nn.Parameter(torch.tensor(-2.0, dtype=torch.float32))
    self.blocks[i].attn.krl_gate = nn.Parameter(torch.tensor(-2.0, dtype=torch.float32))
  self._init_weights()
 ...
 def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
  ...
  q_first: Tensor | None = None
  k_first: Tensor | None = None
  for i in range(self.num_encoder_layers):
   ve = self._get_ve(i, input_ids, ve_cache)
   x, v_raw, q_raw, k_raw = self.blocks[i](x, x0, v_embed=ve,
    v_first=v_first if self.vrl_enabled else None,
    q_first=q_first if self.qkrl_enabled else None,
    k_first=k_first if self.qkrl_enabled else None)
   if i == 0:
    if self.vrl_enabled: v_first = v_raw
    if self.qkrl_enabled: q_first, k_first = q_raw, k_raw
   skips.append(x)
  for i in range(self.num_decoder_layers):
   ...
   x, _, _, _ = self.blocks[bi](x, x0, v_embed=ve,
    v_first=v_first if self.vrl_enabled else None,
    q_first=q_first if self.qkrl_enabled else None,
    k_first=k_first if self.qkrl_enabled else None)
  x = self.final_norm(x)
...
def main() -> None:
 ...
 base_model = GPT(
  ...
  vrl_enabled=args.vrl_enabled, qkrl_enabled=args.qkrl_enabled,
 ).to(device).bfloat16()
