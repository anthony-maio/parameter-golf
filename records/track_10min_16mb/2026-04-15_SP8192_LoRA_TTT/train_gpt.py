import collections, copy, fcntl, glob, io, json, lzma, math, os
try:
    from fused_softcap_ce import fused_softcap_cross_entropy_per_token as _fused_ce
    _HAS_FUSED_CE = True
except ImportError:
    _HAS_FUSED_CE = False
...
class GPT(nn.Module):
    ...
    def forward_logits(self, input_ids):
        ...
        return logits_proj if getattr(self, '_raw_logits', False) else self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
    ...
    def forward_ttt(self, input_ids, target_ids, lora):
        ...
        dec_iter = self.decoder_indices if self.looping_active else range(self.num_encoder_layers, self.num_encoder_layers + self.num_decoder_layers)
        slot = 0
        def _lora_at(name_list, s):
            return name_list[s] if name_list is not None else None
        for i in enc_iter:
            x = self.blocks[i](x, x0,
                               lora_q=_lora_at(lora.q_loras, slot),
...
def eval_val_sliding(h, device, val_data, base_model, batch_seqs=32):
    base_model.eval()
    base_model._raw_logits = _HAS_FUSED_CE
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            nll = _fused_ce(logits.reshape(-1, logits.size(-1)).float(), y_batch.reshape(-1), h.logit_softcap).reshape(bsz, seq_len) if _HAS_FUSED_CE and getattr(base_model, '_raw_logits', False) else F.cross_entropy(logits.reshape(-1, logits.size(-1)).float(), y_batch.reshape(-1), reduction='none').reshape(bsz, seq_len)
            for i, ws in enumerate(batch_ws):
                wlen = wlens[i]
...
def _compute_chunk_window(ci, pred_len, num_chunks, chunk_size, ev_seq_len):
    chunk_end = pred_len if ci == num_chunks - 1 else (ci + 1) * chunk_size
    win_start = max(0, chunk_end - ev_seq_len)
...
def _accumulate_bpb_lora(ptl, x, y, chunk_offsets, chunk_lens, pos_idx,
...
def ttt_lora_evaluate(h, base_model, device, val_data, forward_ttt_train):
    global BOS_ID
    if BOS_ID is None:
        BOS_ID = 1
    ...
    log(f'ttt_lora:docs:{len(doc_entries)} rank:{h.ttt_lora_rank} lr:{h.ttt_lora_lr} chunk:{h.ttt_chunk_size}')
    chunk_size, ev_seq_len = h.ttt_chunk_size, h.ttt_eval_seq_len
    global_batches_sorted = _build_ttt_global_batches(doc_entries, h, ascending=False)
    ...
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    reusable_lora = BatchedTTTLoRA(h.ttt_batch_size, base_model, h.ttt_lora_rank,
                                   k_lora=h.ttt_k_lora, mlp_lora=h.ttt_mlp_lora, o_lora=h.ttt_o_lora).to(device)

    def _build_opt(lora):
        if h.ttt_optimizer == 'sgd':
            return torch.optim.SGD(lora.parameters(), lr=h.ttt_lora_lr, momentum=h.ttt_beta1, weight_decay=h.ttt_weight_decay)
        return torch.optim.AdamW(lora.parameters(), lr=h.ttt_lora_lr, betas=(h.ttt_beta1, h.ttt_beta2),
    ...
    while True:
        ...
        if bsz == reusable_lora.bsz:
            ...
            cur_lora = BatchedTTTLoRA(bsz, base_model, h.ttt_lora_rank,
                                      k_lora=h.ttt_k_lora, mlp_lora=h.ttt_mlp_lora, o_lora=h.ttt_o_lora).to(device)
        ...
        for ci in range(max_nc):
            ...
            for b in range(bsz):
                ...
                win_start, win_len, chunk_offset, chunk_len = _compute_chunk_window(
                    ci, pred_lens[b], num_chunks[b], chunk_size, ev_seq_len)
                tok_starts[b] = doc_start + win_start
                tok_wls[b] = win_len
                chunk_offsets_cpu[b] = chunk_offset
                chunk_lens_cpu[b] = chunk_len
            _, context_size, chunk_offset, _ = _compute_chunk_window(ci, (ci + 1) * chunk_size, ci + 1, chunk_size, ev_seq_len)
            ...
            with torch.no_grad():
                _accumulate_bpb_lora(per_tok_loss, x, y, chunk_offsets, chunk_lens, ctx_pos,
                                     val_data.base_bytes_lut, val_data.has_leading_space_lut,
                                     val_data.is_boundary_token_lut, loss_sum, byte_sum, token_count)
            if needs_train:
